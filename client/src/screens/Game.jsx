...
const STATUS_INFO = {
  ...
  apprivoise: { icon: "🔥", label: "ฉันคว้ามันได้แล้ว", cls: "bg-echo-hp", desc: "ร่างทาวเบิร์น: ปลดล็อกสกิลพื้นฐาน 2/สกิลรอง/ท่าไม้ตาย พลังโจมตีถาวร +1 — คงอยู่ 10 เทิร์น หมดแล้วกลับเป็นทาคุโตะปกติ ต้องเก็บดวงดาวให้ครบ 5 อีกครั้งเพื่อแปลงร่างใหม่ (ยกเว้นกันตายสกิลติดตัว 1 ทำงานแล้ว — ร่างนี้จะถาวรไม่หมดเวลาอีกเลย)" },
            ...
            {items.map((it, i) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm opacity-80">แต้มการ์ด +{it.cost}</div>
                </div>
              </button>
            ))}
          </div>
...
export default function Game({ state, lowQ }) {
  ...
  useEffect(() => {
    if (lwCardBurnOpen && (phase !== "PLAYING" || done)) setLwCardBurnOpen(false);
  }, [lwCardBurnOpen, phase, done]);
  useEffect(() => {
        ...
        <div className={`shrink-0 max-h-[36vh] overflow-y-auto grid gap-2 px-2 pt-2 ${others.length <= 1 ? "grid-cols-1 max-w-sm w-full mx-auto" : "grid-cols-2"}`}>
          {others.map((p) => (
            <MobileOpponent
              ...
              p={p}
              phase={phase}
              targetable={((iAmAttacker && !p.statuses?.seal) || !!anataSel || dawnSel || nightSel || appleSel || bbSel || shSel || skSel || doomSel || saObSel || saLocaSel || bgSel || kawaiiSel || !!bardPending || nanayaSel || lenCopySel || lwStealSel || tpSel) && p.alive}
              picked={!!anataSel && anataSel.includes(p.id)}
              onAttack={(id) => (anataSel ? pickAnata(id) : dawnSel ? pickDawn(id) : nightSel ? pickNight(id) : appleSel ? pickGive(id) : bbSel ? pickBb(id) : shSel ? pickSh(id) : skSel ? pickSk(id) : doomSel ? pickDoom(id) : saObSel ? pickSaOb(id) : saLocaSel ? pickSaLoca(id) : bgSel ? pickBg(id) : kawaiiSel ? pickKawaii(id) : bardPending ? pickBard(id) : nanayaSel ? pickNanaya(id) : lenCopySel ? pickLenCopy(id) : lwStealSel ? pickLwSteal(id) : tpSel ? pickTp(id) : socket.emit("attack", { targetId: id }))}
          ...
          <div className="shrink-0 px-2 pb-2">
            <div className="rounded-3xl p-3 shadow-2xl" style={{ background: "linear-gradient(135deg,#7a2230,#a3283a)" }}>
              ...
              {tepeuPonderLocked && phase === "PLAYING" && !done && (
                <div className="text-center text-sm font-bold text-echo-hp mt-1">🤔 เป็นแบบนี้นี่เอง — ครุ่นคิดอยู่ จั่วไพ่ไม่ได้ (ชนะจั่วยังโจมตีได้)</div>
              )}
                ...
                {phase === "PLAYING" && me.alive && !done ? (
                  <>
                    <div className="flex gap-2">
                      <Button variant="cyan" className="flex-1 py-4 text-xl" disabled={me.atCap || noDraw || shCharging || rgCharging || phenexTaunting || tepeuPonderLocked} onClick={() => { clickSound(); socket.emit("hit"); }}>🎴 จั่วการ์ด</Button>
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-3 gap-3 mt-2">
                  <SkillSlot label="สกิลพื้นฐาน" tier="basic" skill={ch?.basic} points={me.skillPoints} disabled={done || phase !== "PLAYING" || noSkill || moonCellOn || miyakoHealPending || hakunoSecondaryPending || beatBasicLocked || shCharging || rgCharging || phenexTaunting || bardNoteLocked || (me.skillUsed && !mageRepeat && !gambleRepeat && !isApple && !isAquarion && !isBard && !isTohno && !isHakuno && !isDoomguy) || mageLocked || cassiusLocked || veilLocked || ktBasicLocked || (isHakuno && me.hakunoGenderSwitched) || doomBasicLocked || takutoBasicPending || tepeuCookLocked} onUse={skill} ammo={isGambler ? me.gamblerUses : undefined} cost={isGambler && goldenOn ? halfCost(ch?.basic) : isKotone && overworkMe ? ktCost(ch?.basic) : undefined} />
                  <SkillSlot label="สกิลรอง" tier="secondary" skill={ch?.secondary} points={me.skillPoints} disabled={lwSelectMode ? false : (done || phase !== "PLAYING" || noSkill || moonCellOn || miyakoComboPending || hakunoSecondaryPending || (me.skillUsed && !isBard && !isDoomguy) || shCharging || rgCharging || phenexTaunting || bardNoteLocked || ohgerLocked || mysticLocked || lanLocked || ktSecLocked || skSecLocked || banagherAssaultLocked || doomNoEffectLocked || takutoSecPending || takutoNotApprivoiseLocked || monsterMe || tepeuPonderLocked || tepeuCookLocked)} onUse={skill} ammo={isApple ? me.appleGiveUses : me.beamAmmo} cost={lwSelectMode ? 0 : isGambler && goldenOn ? halfCost(ch?.secondary) : isKotone && overworkMe ? ktCost(ch?.secondary) : undefined} />
                  {isBard ? <BardComposeSlot me={me} /> : <SkillSlot label="ท่าไม้ตาย" tier="ultimate" skill={ch?.ultimate} points={me.skillPoints} disabled={lenSelectMode ? false : aquaCancelable ? false : (done || phase !== "PLAYING" || noSkill || moonCellOn || beatMe || (me.skillUsed && !lwArcRepeatable) || ultimateActive || monsterMe || humanityLocked || fourthLocked || doomUltLocked || takutoUlt2Locked || tepeuCookLocked || offerLocked || ktUltLocked || aquaUltLocked || shUltLocked || shCharging || rgCharging || phenexTaunting)} onUse={skill} cost={(lenSelectMode || lwArcSelectMode) ? 0 : undefined} />}
                </div>
                {noSkill && phase === "PLAYING" && !done && (
              ...
              <div className="flex flex-col gap-3 justify-center shrink-0 w-32">
                ...
                {phase === "PLAYING" && me.alive && !done ? (
                  <>
                    ...
                    <Button variant="cyan" className="px-3 py-4 text-lg" disabled={me.atCap || noDraw || shCharging || rgCharging || phenexTaunting || tepeuPonderLocked} onClick={() => { clickSound(); socket.emit("hit"); }}>จั่วการ์ด</Button>
