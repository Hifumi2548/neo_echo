...
//  ที่ delegate เข้ามาที่นี่ทั้งหมด จุดเรียกทั้ง ~30 จุดจึงไม่ต้องแก้ | fourthAtk/spearAtk contribution ใน
//  สูตรดาเมจรวม doAttack (~800 บรรทัด shared pipeline) ยังอยู่นอกขอบเขต — ต้องรอ "contribution list" redesign
// ============================================================
...
module.exports = {
  ...
  // เรียกจาก server.js's eva3Active(p) (thin wrapper) — สกิลติดตัว 3 อย่าให้ฉันทำแบบนี้เลย: เลือด <= 4 = ทำงาน
  isEva3Active(engine, p) {
    return !!p && p.alive && p.hp > 0 && p.hp <= EVA13_HP_THRESHOLD && !engine.passiveSealed(p);
  },

  // เรียกจาก server.js's maybeEva3(p) (thin wrapper) — เข้าสกิลติดตัว 3 ครั้งแรก: เล่นวีดีโอ + ฟื้นเกราะทันที
  ...
  // เรียกจาก server.js's evaLossImmune(p) (thin wrapper) — สกิลติดตัว 2 ทุกอย่างไร้ความหมาย: ไม่รับดาเมจแพ้จั่ว/แตก
  ...
  // เรียกจาก dealDirect()/dealMixed() (thin wrapper eva13RsHopperBlock) — สกิล/เลือกเป้าหมาย (ไม่ใช่โจมตีปกติ) -> กันความเสียหายเต็ม 100%
  ...
  // เรียกจาก dealDirect()/dealMixed() (thin wrapper eva13NormalAttackFloor) — การโจมตีปกติ: ตรึงพลังชีวิตไว้ที่ EVA13_HP_THRESHOLD
