...
const APPLE_ATK_DURATION = 3; // patch 2.2.3: แต่ละหน่วยคงอยู่ 3 เทิร์นแยกกัน (คนละก้อนกัน) (เดิม 5)
...
// ตัวตนของเล็น/ไวท์เล็น (patch 2.2 beta: เลิกสลับ characterId ตอนใช้ท่าที่คัดลอกมาแล้ว — ฟังก์ชันนี้จึงเท่ากับ p.characterId เสมอ
//  แต่ยังคงไว้เป็นจุดเรียกเดียวที่ชัดเจนสำหรับโค้ดที่ต้องการ "ตัวตนของเล็น/ไวท์เล็น" — กันสับสนว่าทำไมถึงไม่ใช้ p.characterId ตรงๆ)
function lenTrueId(p) {
  return p && p.characterId;
}
...
function castBorrowedSkill(p, entry) {
  ...
  if (!skill) return;
  // (patch 2.2 beta แก้บั๊ก) เดิมสลับ p.characterId ชั่วคราวเพื่อให้โบนัสที่ผูกกับ characterId ของเจ้าของท่า
  //  ทำงานด้วย แต่ผลข้างเคียงคือ "สกิลติดตัว" (passive) ของเจ้าของท่าติดมาด้วยทั้งที่ไม่ควร (เช่น Oberon's
  //  ATK=0 เสมอ, Eva13's กันดาเมจแตก/จั่ว ฯลฯ ทำงานกับเล็น/ไวท์เล็นไปด้วยทั้งที่ไม่ได้ตั้งใจ) — และเสี่ยงหลุด
  //  ไปโผล่เป็นชื่อ/ภาพเจ้าของท่าได้ถ้ามีจุดใดอ้าง p.characterId ตรงๆ โดยไม่ผ่าน lenTrueId — เอาออกแล้ว
  //  ตอนนี้ใช้แค่ effect ที่ประกาศไว้ (heal/armor/points/shield/status ฯลฯ) เท่านั้น ไม่สวมร่างจริงอีกต่อไป
...
function applyBardSong(p, pattern, targets) {
  ...
  switch (pattern) {
    case "RRR": // Encore: หลบหลีก +100% ในการโดนโจมตี 1 ครั้งถัดไป (ซ้อนทับได้สูงสุด 3 — patch 2.0.6.1)
      if (!t) return;
      t.statuses.evade = Math.min(BARD_EVADE_MAX, (t.statuses.evade || 0) + 1); // หมดทีละ 1 เมื่อถูกเลือกโจมตี — ไม่ใช้ 3 เทิร์นหมดเอง (patch 2.1.7)
      t.evadeIdle = 0;
    ...
    case "RJR": // Fate's Prelude: โชคลาภในการจั่วไพ่ครั้งถัดไป (ซ้อนทับได้สูงสุด 3 — patch 2.0.6.1)
      if (!t) return;
      t.statuses.fortune = Math.min(BARD_FORTUNE_MAX, (t.statuses.fortune || 0) + 1); // หมดทีละ 1 ต่อการจั่ว — ไม่ใช้ 3 เทิร์นหมดเอง (patch 2.1.7)
      t.fortuneIdle = 0;
...
// "ไม่ใช้งานต่อ" / "ไร้ทางเยียวยา" (สถานะ Universal patch 2.2.1): ฟื้นเลือดจริงไม่ได้
function noHealActive(p) {
  return !!p && (((p.statuses && p.statuses.nohealing) || 0) > 0 || ((p.statuses && p.statuses.norecover) || 0) > 0);
}
...
const NANAYA_REST_TURNS = 2;        // พักผ่อนสักครู่: ทุกๆ 2 เทิร์น ฟื้นพลังชีวิต
...
const HAKUNO_NORECOVER_TURNS = 3;     // ข้าขอบัญชา (หญิง) / MOON*CELL: ติดไร้ทางเยียวยา 3 เทิร์น
const HAKUNO_FORCE_SCORE_2 = 19;      // ข้าขอบัญชา (หญิง): บังคับแต้มการจั่วเป็น 19 ทันที
const HAKUNO_DRAW_LOW_VALUES = [2, 3]; // ข้าขอบัญชา (หญิง): จั่วเพิ่มระหว่างนี้ได้แค่ 2 หรือ 3 แต้ม
const HAKUNO_MOONCELL_NEED = 3;       // MOON*CELL: ต้องมีแต้มคำสาปแห่งดวงจันทร์ครบ 3 ต่อการเปิด 1 ครั้ง
const HAKUNO_MOONCELL_TURNS = 5;      // MOON*CELL: คงอยู่ 5 เทิร์น
const HAKUNO_MOONCELL_ATK_BONUS = 1;  // MOON*CELL: พลังโจมตีรวมสูงสุด 2 หน่วยเท่ากันทั้งสองร่าง (ระงับ +1 ถาวรร่างชายระหว่างนี้)
const HAKUNO_COMMAND_USES = 3;        // อาคมบัญชาระดับ EX+: ใช้ได้ 3 ครั้งต่อเกม
...
const PHENEX_NTD_ATK_BONUS = 1;   // ฝืนใช้งาน NTD-Sytem: พลังโจมตีพื้นฐาน +1 ระหว่าง NTD ทำงาน (รวมเป็นโจมตีปกติ 2) — ซ้อนกับสกิลติดตัว 1 ได้อีก +1 (ตายแล้วเกิดใหม่) รวมเป็น 3 ถาวร (แก้บั๊ก patch 2.1.9: เดิม +2 ทำให้รวมพลังโจมตีถึง 4)
...
const PHENEX_NTD_IMG = "/characters/rita/profile/phenex_ntd.png";  // ระหว่างฝืนใช้งาน NTD-Sytem (ชั่วคราวหรือถาวรหลังสกิลติดตัว 1)
...
const TRANSFORMS = {
  ...
  tohnoKill: { img: TOHNO_DEATH_IMG, video: "/characters/tohno/tohno_passive_hit.mp4", title: "Mystic eye of death perception", label: "สังหารด้วยเนตรมาร", seconds: 7, music: null, afterReveal: false },
...
function drawCardFor(p) {
  ...
  // ข้าขอบัญชา (หญิง คิชินามิ ฮาคุโนะ patch 2.2.1): จั่วเพิ่มระหว่างนี้ได้แค่ 2 หรือ 3 แต้มเท่านั้น
  if (p.hakunoLowDraw) {
    const low = avail.filter((v) => HAKUNO_DRAW_LOW_VALUES.includes(v));
    if (low.length) avail = low;
  }
...
function loseArmor(p) {
  ...
  // ไม่อยากให้ใครต้องเจ็บปวด (ท่าไม้ตาย 2 ริต้า เบอร์นัล patch 2.1.6): ระหว่างล่อเป้า สะสม "ความเจ็บปวด" +1 ทุกๆ 1 หน่วยเกราะที่เสียไป
  if (p.characterId === "phenex" && (p.statuses.phenexTaunt || 0) > 0) p.phenexPain = (p.phenexPain || 0) + 1;
  if (!linkMirror) {
    const b = linkedBuddyOf(p);
...
function startMatch() {
  ...
  dayForceUntil = 0;
  allyWinFlag = false;
  // อาริมะ มิยาโกะ: เจอ โทโนะ ชิกิ หรือ นานายะ ชิกิ ในเกมเดียวกัน -> เล่นวีดีโอ arima_shiki.mp4 ก่อนเริ่มเทิร์นแรก
  const miyako = Object.values(players).find((p) => p.characterId === "miyako");
  const hasShikiRival = miyako && Object.values(players).some((p) => p.id !== miyako.id && (p.characterId === "tohno" || p.characterId === "nanaya"));
  if (hasShikiRival) {
    cutsceneQueue = [];
    queueCutscene(miyako, "arimaShiki");
...
function dealRound() {
  ...
  for (const p of Object.values(players)) {
    ...
    if (p.characterId === "nanaya" && !passiveSealed(p)) {
      // พักผ่อนสักครู่ (สกิลติดตัว 3): ทุกๆ 2 เทิร์น ฟื้นพลังชีวิต
      p.nanayaRestTurn = (p.nanayaRestTurn || 0) + 1;
      if (p.nanayaRestTurn >= NANAYA_REST_TURNS) {
        p.nanayaRestTurn = 0;
        const heal = healHp(p, NANAYA_REST_HEAL);
        if (heal > 0) lastLog.push(`💤 ${p.name} พักผ่อนสักครู่ — ฟื้นพลังชีวิต +${heal}`);
      }
    }

    // ---------- คิชินามิ ฮาคุโนะ (patch 2.2.1) ----------
    p.hakunoGenderSwitched = false; // เธอ/นาย คือฉันหรอ?: สลับเพศได้อีก 1 ครั้งในเทิร์นใหม่นี้
    if (p.characterId === "hakuno" && p.hakunoGender === "male") {
      // ร่างชาย: ทุกๆ 2 เทิร์น ฟื้นพลังชีวิต +1
      p.hakunoRestTurn = (p.hakunoRestTurn || 0) + 1;
      if (p.hakunoRestTurn >= HAKUNO_MALE_REST_TURNS) {
        p.hakunoRestTurn = 0;
        const heal = healHp(p, HAKUNO_MALE_REST_HEAL);
        if (heal > 0) lastLog.push(`💗 ${p.name} ร่างชาย — ฟื้นพลังชีวิต +${heal}`);
      }
    }
    ...
    if ((p.statuses.hburn || 0) > 0 && p.alive) {
      ...
      // ขอแค่ได้พบกันอีก (สกิลติดตัว 2 ริต้า เบอร์นัล patch 2.1.6): ลุกไหม้ที่เกิดกับตัวเองรักษาแทนสร้างความเสียหายเสมอ (ไม่ต้องมีเงื่อนไข)
      if ((p.characterId === "hikaru" && (p.statuses.gingastrium || 0) > 0) || p.characterId === "phenex") {
        const heal = healHp(p, 1);
        lastLog.push(`❤️‍🔥 ${p.name} ${p.characterId === "phenex" ? "ขอแค่ได้พบกันอีก" : "หัวใจที่ลุกไหม้"} — ลุกไหม้กลายเป็นการรักษา ฟื้นพลังชีวิต +${heal} (เหลืออีก ${p.statuses.hburn - 1} หน่วย)`);
      } else {
  ...
  if (roundNumber > 1 && night !== prevNight) {
    ...
    if (night) {
      for (const p of alivePlayers()) {
        ...
        // Arc Drive finish (ไวท์เล็น patch 2.2 beta): พลังโจมตีสะสมหมดอายุทุกครั้งที่เปลี่ยนช่วงเวลา (วัน↔คืน)
        if (p.characterId === "lenwhite" && (p.arcAtkBurns || 0) > 0) {
          p.arcAtkBurns = 0;
          lastLog.push(`🌙 ${p.name} Arc Drive finish — พลังโจมตีที่สะสมไว้หมดอายุลงเมื่อกลางคืนมาถึง`);
        }
      }
    } else {
      for (const p of alivePlayers()) {
...
// นานายะ ชิกิ: เปิด/ปิด Mystic eye of death perception (ค่าเริ่มต้นปิด — เปิด/ปิดได้แค่ 1 ครั้งต่อเทิร์น)
function nanayaToggleEye(id) {
  const p = players[id];
  if (gameState !== "PLAYING" || !p || !p.alive || p.locked) return;
  if (p.characterId !== "nanaya") return;
  if (passiveSealed(p)) return; // อันนี้ของนายรึเปล่า: สกิลติดตัวไม่ทำงาน — เปิด/ปิดไม่ได้
  if (p.nanayaToggleUsed) return;
  p.nanayaToggleUsed = true;
  p.nanayaEyeOn = !p.nanayaEyeOn;
  if (p.nanayaEyeOn) p.transformAt = ++transformCounter; // เพลง/ฉากหลังเริ่มใหม่ทุกครั้งที่เปิด
  lastLog.push(`👁️ ${p.name} Mystic eye of death perception — ${p.nanayaEyeOn ? "เปิดใช้งาน" : "ปิดใช้งาน"}`);
  io.emit("skillFlash", {
...
function useSkill(id, tier, targets, item) {
  ...
  // ---------- นานายะ ชิกิ: อันนี้ของนายรึเปล่า — เลือกเป้าหมาย 1 คน (คนอื่นเท่านั้น) ----------
  ...
  if (isGambler) {
    ...
    if (tier === "basic") {
      ...
      if (win) {
        ...
        if (p.hp <= 0) {
          instantDeath(p);
          if (!p.alive) lastLog.push(`💀 ${p.name} เสี่ยงดวงจนสิ้นลม — ตกรอบทันที!`);
        }
      }
  ...
  if (isHakunoGender) {
    ...
    p.transformAt = ++transformCounter;
    flashSuffix = ` — เปลี่ยนเป็นคิชินามิ ฮาคุโนะ (${newGender === "male" ? "ชาย" : "หญิง"})`;
    lastLog.push(`🌓 ${p.name} เธอ/นาย คือฉันหรอ? — เปลี่ยนเป็นร่าง${newGender === "male" ? "ชาย" : "หญิง"}`);
  }
  ...
  // ---------- นานายะ ชิกิ: อันนี้ของนายรึเปล่า — ห้ามใช้สกิล/จั่วไพ่/สกิลติดตัวไม่ทำงาน 2 เทิร์น ----------
  //  ทุกครั้งที่ใช้ เสียพลังชีวิตตัวเอง 1 หน่วยไม่สนเกราะ (แม้ผลจะถูกลบล้างก็ยังเสีย)
  if (isNanayaSilence && nanayaSilenceTarget) {
    const t = nanayaSilenceTarget;
    if (satoruOnTargeted(t, p, `สกิล ${skill.name} `).negated) {
      flashSuffix = " — ถูกลบล้าง";
    } else {
      t.statuses.noskill = Math.max(t.statuses.noskill || 0, NANAYA_SILENCE_TURNS);
      t.statuses.nodraw = Math.max(t.statuses.nodraw || 0, NANAYA_SILENCE_TURNS);
      t.statuses.nanayaSeal = Math.max(t.statuses.nanayaSeal || 0, NANAYA_SILENCE_TURNS);
      flashSuffix = ` — ใส่ ${t.name}`;
      lastLog.push(`👁️ ${p.name} อันนี้ของนายรึเปล่า — ${t.name} ใช้สกิล/จั่วไพ่ไม่ได้ และสกิลติดตัวไม่ทำงาน ${NANAYA_SILENCE_TURNS} เทิร์น`);
    }
    dealDirect(p, NANAYA_SILENCE_SELF_DMG);
    lastLog.push(`💢 ${p.name} เสียพลังชีวิต ${NANAYA_SILENCE_SELF_DMG} หน่วย (ไม่สนเกราะ) จากการใช้อันนี้ของนายรึเปล่า`);
    if (p.alive && p.hp <= 0) { instantDeath(p); if (!p.alive) lastLog.push(`💀 ${p.name} เลือดจริงหมด ตกรอบ!`); }
  }
  ...
  if (isTakutoUlt2) {
    ...
    if (scoreOf(p) !== 21) {
      ...
      lastLog.push(`👁️ ${p.name} อย่างนายน่ะ จะไปเข้าใจอะไร — แต้มการ์ดของตัวเองพุ่งเป็น ${TAKUTO_ULT_CARD_SCORE} แต้มทันที และการโจมตีคอมโบครั้งนี้มีโอกาสได้โจมตีเพิ่มเป็นครั้งที่ 3 (${Math.round(TAKUTO_THIRD_ATK_CHANCE * 100)}%)`);
    } else {
  ...
  // ---------- ริต้า เบอร์นัล / ฟีนิกซ์ (patch 2.1.6) ----------
  // หัวใจที่ไม่ดับมอด: ติดลุกไหม้ตัวเอง +3 หน่วย (สะสมสูงสุด 5) — phenexIgnite เป็น trigger เฉยๆ ไม่ใช่สถานะจริง (แปลงเป็นสแตค hburn ทันที)
  if (st === "phenexIgnite") {
  ...
  // ฝืนใช้งาน NTD-Sytem: เสียพลังชีวิต 3 หน่วยไม่สนเกราะ แล้วเปิด NTD-System 5 เทิร์น (สกิลรองเปลี่ยนเป็นสกิลรอง 2 ระหว่างนี้)
  if (st === "phenexNtd") {
    triggerCutscene(p, "phenexNtd");
    dealDirect(p, PHENEX_NTD_COST_HP);
    maybeBeatSave(p); maybeBeatMode(p); maybeEva3(p); maybeWakeKotone(p);
    lastLog.push(`⚙️ ${p.name} ฝืนใช้งาน NTD-Sytem — เสียพลังชีวิต ${PHENEX_NTD_COST_HP} หน่วย (ไม่สนเกราะ) เปิดใช้งาน NTD-System ${p.statuses.phenexNtd || 0} เทิร์น (สกิลรองเปลี่ยนเป็นอย่าอยู่เลย แกน่ะ!)`);
    if (p.alive && p.hp <= 0) { instantDeath(p); if (!p.alive) lastLog.push(`💀 ${p.name} เลือดจริงหมด ตกรอบ!`); }
  }
  // ไม่อยากให้ใครต้องเจ็บปวด: ล่อเป้าทุกคนมาที่ตัวเอง 3 เทิร์น (redirect ใน doAttack) — ไม่มีค่าใช้จ่ายเลือด
  if (st === "phenexTaunt") {
    triggerCutscene(p, "phenexTaunt");
    lastLog.push(`🥺 ${p.name} ไม่อยากให้ใครต้องเจ็บปวด — ล่อเป้าหมายการโจมตีของทุกคนมาที่ตัวเอง ${p.statuses.phenexTaunt || 0} เทิร์น (ใช้สกิล/จั่วไพ่ไม่ได้ระหว่างนี้ แต่ยังโจมตีได้ถ้าชนะ)`);
  }
  // อย่าอยู่เลย แกน่ะ!: ทันทีที่ใช้ แต้มการจั่วกลายเป็น 20 แต้ม (ไม่มีผลถ้ามีแต้ม 21 อยู่แล้ว) — ผลต่อการโจมตีดูใน doAttack
  if (st === "phenexPurge") {
    if (scoreOf(p) !== 21) {
      p.cardBonus = PHENEX_FORCE_SCORE - calculateScore(p.cards);
      p.busted = bustedOf(p);
      lastLog.push(`🎯 ${p.name} อย่าอยู่เลย แกน่ะ! — แต้มการจั่วกลายเป็น ${PHENEX_FORCE_SCORE} แต้มทันที`);
    } else {
      lastLog.push(`🎯 ${p.name} อย่าอยู่เลย แกน่ะ! — แต้ม 21 อยู่แล้ว ไม่มีผลกับแต้มการจั่ว`);
    }
  }
  // ---------- อาริมะ มิยาโกะ (patch 2.2.0) ----------
  // พี่จ๋าอยู่ไหน: การโจมตีปกติครั้งถัดไปฟื้นเลือด +1 — คงอยู่จนกว่าจะได้โจมตี (ใช้ร่วมกับเพลงหมัด อาริมะได้)
  if (st === "miyakoHeal") {
    lastLog.push(`💗 ${p.name} พี่จ๋าอยู่ไหน — การโจมตีปกติครั้งถัดไปจะฟื้นพลังชีวิต +${MIYAKO_HEAL_AMOUNT}`);
  }
  // เพลงหมัด อาริมะ: การโจมตีปกติครั้งถัดไปต่อคอมโบได้สูงสุด 4 ครั้ง (โอกาสลดลงเป็นขั้น) — คงอยู่จนกว่าจะได้โจมตี
  if (st === "miyakoCombo") {
    p.miyakoComboHits = 1;
    lastLog.push(`🥊 ${p.name} เพลงหมัด อาริมะ — การโจมตีปกติครั้งถัดไปต่อคอมโบได้สูงสุด 4 ครั้ง (100%/75%/50%/25%)`);
  }
  // หนูจะทำให้พี่ตาสว่างเอง: ทันทีที่ใช้ แต้มการจั่วกลายเป็น 20 แต้ม (ไม่มีผลถ้ามีแต้ม 21 อยู่แล้ว) — ผลต่อการโจมตีดูใน doAttack
  if (st === "miyakoUlt") {
    if (scoreOf(p) !== 21) {
      p.cardBonus = MIYAKO_FORCE_SCORE - calculateScore(p.cards);
      p.busted = bustedOf(p);
      lastLog.push(`🎯 ${p.name} หนูจะทำให้พี่ตาสว่างเอง — แต้มการจั่วกลายเป็น ${MIYAKO_FORCE_SCORE} แต้มทันที`);
    } else {
  ...
  if (st === "hakunoInvertReady") {
    ...
    lastLog.push(`🌓 ${p.name} ข้าขอบัญชา — การโจมตีปกติครั้งถัดไปจะทำให้เป้าหมายติดผกผัน ${HAKUNO_INVERT_TURNS} เทิร์น (แต้มคำสาปแห่งดวงจันทร์ ${p.hakunoMoonPoints}/${HAKUNO_MOONCELL_NEED})`);
  }
  // ข้าขอบัญชา (หญิง): บังคับแต้มการจั่วเป็น 19 ทันที + จั่วเพิ่มได้แค่ 2/3 แต้ม — โจมตีปกติครั้งถัดไปเป้าหมายเกราะไม่ฟื้น+ไร้ทางเยียวยา 3 เทิร์น
  if (st === "hakunoNoRegenReady") {
    p.hakunoMoonPoints = (p.hakunoMoonPoints || 0) + 1;
    p.hakunoLowDraw = true;
    if (scoreOf(p) !== HAKUNO_FORCE_SCORE_2) {
      p.cardBonus = HAKUNO_FORCE_SCORE_2 - calculateScore(p.cards);
      p.busted = bustedOf(p);
      lastLog.push(`🌕 ${p.name} ข้าขอบัญชา — แต้มการจั่วกลายเป็น ${HAKUNO_FORCE_SCORE_2} แต้มทันที จั่วเพิ่มได้อีกแต่จะได้แค่ 2 หรือ 3 แต้มเท่านั้น (แต้มคำสาปแห่งดวงจันทร์ ${p.hakunoMoonPoints}/${HAKUNO_MOONCELL_NEED})`);
    } else {
      lastLog.push(`🌕 ${p.name} ข้าขอบัญชา — แต้ม ${HAKUNO_FORCE_SCORE_2} อยู่แล้ว จั่วเพิ่มได้อีกแต่จะได้แค่ 2 หรือ 3 แต้มเท่านั้น (แต้มคำสาปแห่งดวงจันทร์ ${p.hakunoMoonPoints}/${HAKUNO_MOONCELL_NEED})`);
    }
  }
  // MOON*CELL: เสียแต้มคำสาปแห่งดวงจันทร์ 3 — ล้าง/ปิดใช้งานบัฟ-ดีบัฟ-สกิล-สกิลติดตัวของทุกคนยกเว้นตัวเอง 5 เทิร์น
  if (st === "moonCell") {
    ...
    triggerCutscene(p, p.hakunoGender === "female" ? "hakunoUltFemale" : "hakunoUltMale");
    lastLog.push(`🌙 ${p.name} คำสาปแห่งดวงจันทร์ MOON*CELL — ล้างบัฟ/ดีบัฟและปิดสกิล/สกิลติดตัวของทุกคน (ยกเว้นตัวเอง) เป็นเวลา ${HAKUNO_MOONCELL_TURNS} เทิร์น! เกราะทุกคนจะไม่ฟื้นระหว่างนี้`);
  }
...
function hakunoCommandSpell(id, command) {
  ...
  let what = "";
  if (cmd === 1) {
    p.skillPoints = maxSkillOf(p);
    what = "เติมแต้มสกิลเต็ม";
    lastLog.push(`📜 ${p.name} อาคมบัญชาระดับ EX+ — เติมแต้มสกิลเต็ม ${maxSkillOf(p)} แต้ม (เหลือ ${p.hakunoCommandUses})`);
  } else if (cmd === 2) {
    const heal = healHp(p, maxHpOf(p));
    what = "ฟื้นพลังชีวิตเต็ม";
    lastLog.push(`📜 ${p.name} อาคมบัญชาระดับ EX+ — ฟื้นพลังชีวิตเต็ม${heal > 0 ? ` (+${heal})` : ""} (เหลือ ${p.hakunoCommandUses})`);
  } else {
    if (scoreOf(p) !== 21) {
      p.cardBonus = 21 - calculateScore(p.cards);
      p.busted = bustedOf(p);
    }
    what = "บังคับแต้มการ์ดเป็น 21";
    lastLog.push(`📜 ${p.name} อาคมบัญชาระดับ EX+ — แต้มการ์ดกลายเป็น 21 ทันที (เหลือ ${p.hakunoCommandUses})`);
  }
...
// หัวใจฆาตกร (นานายะ ชิกิ สกิลติดตัว 2 patch 2.1.9): เนตรมารพลาดสังหาร -> เปิดโอกาสโจมตีซ้ำทันที
...
function postAttackFollowup(attacker) {
  if (attacker && attacker.alive && attacker.characterId === "nanaya" && attacker.nanayaMissedThisAttack) {
  ...
  if (attacker && attacker.alive && attacker.characterId === "miyako" && (attacker.statuses.miyakoCombo || 0) > 0) {
    ...
    if (hitsSoFar < MIYAKO_COMBO_CHANCE.length) {
      const chance = MIYAKO_COMBO_CHANCE[hitsSoFar]; // โอกาสของครั้งถัดไป (ครั้งที่ 2=75% / 3=50% / 4=25%)
      if (Math.random() < chance) {
        const targets = attackableTargets(attacker.id);
        if (targets.length > 0) {
          attacker.miyakoComboHits = hitsSoFar + 1;
          lastLog.push(`🥊 ${attacker.name} เพลงหมัด อาริมะ — ต่อคอมโบสำเร็จ! โจมตีต่อได้อีกครั้ง (ครั้งที่ ${hitsSoFar + 1} โอกาส ${Math.round(chance * 100)}%)`);
          attackerId = attacker.id;
          gameState = "ATTACK";
          startPhaseTimer(ATTACK_TIME, () => {
        ...
        lastLog.push(`🥊 ${attacker.name} เพลงหมัด อาริมะ — คอมโบขาดตอน (โอกาส ${Math.round(chance * 100)}%)`);
      }
    }
    delete attacker.statuses.miyakoCombo;
    attacker.miyakoComboHits = 0;
  ...
  if (attacker && attacker.alive && attacker.characterId === "takuto" && (attacker.statuses.takutoThirdAtk || 0) > 0) {
    ...
    if (Math.random() < TAKUTO_THIRD_ATK_CHANCE) {
      const targets = attackableTargets(attacker.id);
      if (targets.length > 0) {
        lastLog.push(`⚔️ ${attacker.name} อย่างนายน่ะ จะไปเข้าใจอะไร — โจมตีต่อเป็นครั้งที่ 3 สำเร็จ!`);
        attackerId = attacker.id;
        gameState = "ATTACK";
        startPhaseTimer(ATTACK_TIME, () => {
          const t = attackableTargets(attackerId);
      ...
      lastLog.push(`⚔️ ${attacker.name} อย่างนายน่ะ จะไปเข้าใจอะไร — โอกาสโจมตีครั้งที่ 3 ไม่สำเร็จ (${Math.round(TAKUTO_THIRD_ATK_CHANCE * 100)}%)`);
    }
  }
...
function nanayaCancelReattack(id) {
  const p = players[id];
  if (!p || !p.alive || p.characterId !== "nanaya" || !p.nanayaReattackReady) return;
  if (gameState !== "ATTACK" || attackerId !== p.id) return;
  p.nanayaReattackReady = false;
  clearPhaseTimer();
  lastLog.push(`🗡️ ${p.name} หัวใจฆาตกร — ยกเลิกการโจมตีซ้ำ`);
  endTurn();
}
...
function doAttack(byId, targetId) {
  ...
  {
    const phenexTaunter = alivePlayers().find(
      (r) => r.id !== attacker.id && r.characterId === "phenex" && (r.statuses.phenexTaunt || 0) > 0 && !sealActive(r)
    );
  ...
  if (shikiEye && !killSealed(attacker) && (target.statuses.deathline || 0) >= SHIKI_DEATHLINE_MAX) {
    ...
    instantDeath(target);
    target.wasAttacked = true;
    if (!target.alive) lastLog.push(`👁️💀 เนตรมารแห่งความมรณะ! ${attacker.name} มองเห็นเส้นชีวิตของ ${target.name} — สังหารทันที (บังคับตาย)!`);
    else lastLog.push(`👁️💀 เนตรมารแห่งความมรณะ! ${attacker.name} มองเห็นเส้นชีวิตของ ${target.name} — แต่ ${target.name} เกิดใหม่หนีความตายไปได้!`);
  ...
  if (shikiWither && !killSealed(attacker)) {
    ...
    if (lines > 0 && Math.random() < chance) {
      ...
      instantDeath(target);
      target.wasAttacked = true;
      if (!target.alive) lastLog.push(`🥀💀 ความตายที่โรยรา! ${attacker.name} เด็ดเส้นชีวิตของ ${target.name} (โอกาส ${Math.round(chance * 100)}%) — สังหารทันที!`);
      else lastLog.push(`🥀💀 ความตายที่โรยรา! ${attacker.name} เด็ดเส้นชีวิตของ ${target.name} — แต่ ${target.name} เกิดใหม่หนีความตายไปได้!`);
  ...
  //  เปิดใช้งาน: โจมตีปกติมีโอกาสสังหารทันที 50% — พลาด/สังหารไม่ได้ เสียพลังชีวิตไม่สนเกราะ 6 หน่วย (สังหารสำเร็จไม่เสียเลือด)
  //  พลาด = เปิดโอกาสให้หัวใจฆาตกร (สกิลติดตัว 2) โจมตีซ้ำได้ทันที (ดูจุดจบฟังก์ชัน doAttack)
  attacker.nanayaMissedThisAttack = false;
  if (attacker.characterId === "nanaya" && attacker.nanayaEyeOn && !passiveSealed(attacker) && !killSealed(attacker)) {
    if (Math.random() < miyakoKillChance(target, NANAYA_KILL_CHANCE)) {
      ...
      instantDeath(target);
      target.wasAttacked = true;
      if (!target.alive) lastLog.push(`👁️💀 Mystic eye of death perception — ${attacker.name} มองเห็นเส้นความตายของ ${target.name} (โอกาส 50%) — สังหารทันที!`);
      else lastLog.push(`👁️💀 Mystic eye of death perception — ${attacker.name} มองเห็นเส้นความตายของ ${target.name} — แต่ ${target.name} เกิดใหม่หนีความตายไปได้!`);
      lastAttack = {
        byName: attacker.name, byImg: displayImg(attacker), byColor: POSITION_COLORS[attacker.position] || "#888",
        byDoomWeapon: attacker.characterId === "doomguy" ? attacker.doomWeapon : undefined, // DoomGuy: อาวุธที่ใช้ยิงตอนนี้ (เสียงยิงฝั่ง client)
    ...
    dealDirect(attacker, NANAYA_MISS_DMG);
    lastLog.push(`👁️💢 Mystic eye of death perception พลาด — ${attacker.name} เสียพลังชีวิต ${NANAYA_MISS_DMG} หน่วย (ไม่สนเกราะ)`);
    if (attacker.alive && attacker.hp <= 0) {
      instantDeath(attacker);
      if (!attacker.alive) lastLog.push(`💀 ${attacker.name} เลือดจริงหมด ตกรอบ!`);
    }
    ...
    if (attacker.alive) {
      if (Math.random() < NANAYA_REATTACK_CHANCE) {
        ...
        lastLog.push(`🗡️ ${attacker.name} หัวใจฆาตกร — ความแค้นเปิดโอกาสให้โจมตีซ้ำ!`);
      } else {
        lastLog.push(`🗡️ ${attacker.name} หัวใจฆาตกร — ไม่ได้โอกาสโจมตีซ้ำครั้งนี้`);
      }
    }
  }
  ...
  // ---------- ริต้า เบอร์นัล (patch 2.1.6) ----------
  // อย่าอยู่เลย แกน่ะ! (สกิลรอง 2 ระหว่าง NTD-System): ความเสียหายจากการโจมตีปกติไม่สนเกราะ + ลบ/แบนท่าไม้ตายเป้าหมาย (ดูด้านล่าง)
  const phenexPurgeAtk = attacker.characterId === "phenex" && (attacker.statuses.phenexPurge || 0) > 0;
  // ---------- อาริมะ มิยาโกะ (patch 2.2.0) ----------
  // หนูจะทำให้พี่ตาสว่างเอง: ทำงานหลังเปิดการ์ด — ผลจริงดูตอนสรุปผลการโจมตีด้านล่าง (ปิดความสามารถสังหาร/ดาเมจเสริมทะลุเกราะ)
  const miyakoUltAtk = attacker.characterId === "miyako" && (attacker.statuses.miyakoUlt || 0) > 0;
  // ---------- คิชินามิ ฮาคุโนะ (patch 2.2.1) ----------
  ...
  // ฝืนใช้งาน NTD-Sytem (ท่าไม้ตาย 1 — ระหว่าง NTD ทำงาน ไม่ว่าชั่วคราวหรือถาวรหลังเกิดใหม่): พลังโจมตีพื้นฐาน +1 (รวมเป็นโจมตีปกติ 2)
  ...
  if (hakunoInvertAtk) {
    ...
    if (target.alive) {
      if ((target.statuses.invert || 0) > 0) {
        lastLog.push(`🌓 ${attacker.name} ข้าขอบัญชา — ${target.name} ติดผกผันอยู่แล้ว ระยะเวลาไม่เพิ่มขึ้น สกิลเสียเปล่า`);
      } else {
        target.statuses.invert = HAKUNO_INVERT_TURNS;
        lastLog.push(`🌓 ${attacker.name} ข้าขอบัญชา — ${target.name} ติดสถานะผกผัน ${HAKUNO_INVERT_TURNS} เทิร์น!`);
      }
    }
  }
  // ข้าขอบัญชา (หญิง คิชินามิ ฮาคุโนะ): โจมตีปกติทำให้เป้าหมายเกราะไม่ฟื้น + ไร้ทางเยียวยา 3 เทิร์น
  if (hakunoNoRegenAtk) {
    ...
    if (target.alive) {
      ...
      target.statuses.norecover = Math.max(target.statuses.norecover || 0, HAKUNO_NORECOVER_TURNS);
      lastLog.push(`🌕 ${attacker.name} ข้าขอบัญชา — ${target.name} เกราะฟื้นไม่ได้ และติดสถานะไร้ทางเยียวยา ${HAKUNO_NORECOVER_TURNS} เทิร์น!`);
    }
  }
  if (profitAtk > 0) {
...
function endTurn() {
  ...
  // หลบหลีก / โชคลาภ (Bard patch 2.1.7): ไม่ได้ใช้ 3 เทิร์นติดกัน = หมดฤทธิ์ไปเอง (นับใหม่ทุกครั้งที่ได้เพิ่ม/ถูกใช้)
  for (const p of Object.values(players)) {
    if ((p.statuses.evade || 0) > 0) {
      p.evadeIdle = (p.evadeIdle || 0) + 1;
      if (p.evadeIdle >= 3) {
        delete p.statuses.evade;
        if (p.statusAmt) delete p.statusAmt.evade;
        p.evadeIdle = 0;
        lastLog.push(`💨 ${p.name} หลบหลีกไม่ได้ใช้ 3 เทิร์น — หมดฤทธิ์`);
      }
    } else p.evadeIdle = 0;
    if ((p.statuses.fortune || 0) > 0) {
...
io.on("connection", (socket) => {
  ...
  socket.on("nanayaToggleEye", () => nanayaToggleEye(socket.id)); // นานายะ ชิกิ: เปิด/ปิด Mystic eye of death perception
  socket.on("nanayaCancelReattack", () => nanayaCancelReattack(socket.id)); // นานายะ ชิกิ: ยกเลิกโจมตีซ้ำ (หัวใจฆาตกร)
